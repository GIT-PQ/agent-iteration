...
不适用：从零设计新工件（那是设计指南的事）；与工件行为无关的一次性杂务。
...
- 05 可以就是脚本对比输出加一行结论。验证是确定性 diff 时**由脚本执行，不 spawn 隔离子任务**——脚本没有实现者上下文，"非实现者"条件天然满足，为 P3 spawn 一个子任务是重炮打蚊子；只有涉及语义判断（输出质量、措辞、格式观感）才用 spawn/新会话机制
...
模板在 `assets/templates/`，编号即状态。每份模板头部有使用方式、消费方与职责边界——填写前先读。
...
- **出口守卫**：自检通过后 commit 候选（`[T-001] 摘要`），工件冻结。commit 是检查点，tag 才是定稿。
...
- **交接点【等非实现者】**：验证必须由非实现者在干净上下文执行——自检是快速失败闸，不是验证。机制二选一，优先 spawn 隔离子任务（只给工件 + 02-验收标准，**不给**变更方案和实现期对话）；spawn 不可用或被沙箱拒绝时，立即切换为生成验证交接指令、由用户开新会话执行——不要手工搭建隔离目录树补救，隔离要满足的是"验证者不接触实现者的上下文"，不是物理隔离的文件系统。
- **隔离工作区**：验证在工件仓库**之外**的临时目录里、对候选 commit 的提取副本执行（`git archive` 解包，不用 `git worktree`——它的元数据写进原仓库 `.git/`），不影响原仓库的 git 记录。验证写回工件仓库的产物只有 05 报告一份；提取树、交接指令、中间输出全留临时目录，**删除时机由用户决定**——中间文件可能需要人工查看，验证 agent 不自行清理。
- 三类出口：前进（门禁全过）/ 回跳（回归失败未超限→修改实现；根因误判→问题归因）/ 阻断（关键断言失败不可修复）。
...
- 打 tag（指向验证过的候选 commit，与 05 报告的 hash 一致）→ 追加 changelog 条目（工件仓库 `CHANGELOG.md`，含门禁八条对照）→ 归档检查（任务目录 01-05 齐全）。三者原子化。失败候选不进 changelog。
