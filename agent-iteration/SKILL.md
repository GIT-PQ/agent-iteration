...
**纯笔误不开任务**：代码注释、README 措辞这类不影响任何触发、脚本输出或生成内容的改动，直接修改、提请人 commit、changelog 补一行即可，不占任务编号。判断标准只有一条：改动会不会出现在工件的输出或行为里——会（哪怕只是页眉格式），就至少是 P3，走下面的裁剪闭环；不会，就不是迭代。

## 启动迭代任务

1. **确定工件与其仓库**。后续所有存储都相对工件仓库：迭代日志 `iterations/{任务编号}/`、eval 集 `evals/`、基线库 `results/`、`CHANGELOG.md`。工件自含全部迭代历史。若工件仓库尚未 git 化，提醒用户执行 `git init`（候选冻结、tag、回滚点都依赖 git）；并检查最小目录骨架（工件源码、`iterations/`、`evals/`、`results/`、`CHANGELOG.md` 齐全），建议 `.gitignore` 忽略 `results/*`（基线可由「旧 tag + eval 输入」重放，不入库）。
...
- **出口守卫**：自检通过后**提醒人提交候选**（message：`[任务编号] 摘要`，如 `[T-003] 修复华东区重复输出`；commit 即冻结，是 3.4→3.5 的守卫）——**人确认提交后才放行进 3.5**。commit 是检查点，tag 才是定稿。
...
- **隔离工作区**：验证在工件仓库**之外**的临时目录里、对候选 commit 的提取副本（`git archive`，读操作、skill 自动）执行，不影响原仓库的 git 记录；唯一写回仓库的产物是 05 报告；临时目录的删除时机由用户决定，验证 agent 不自行清理。
...
- **提醒人执行**：打 tag（指向验证过的候选 commit，与 05 报告的 hash 一致）、提交留痕与 changelog 条目（工件仓库 `CHANGELOG.md`，含门禁八条对照）；归档检查（任务目录 01-05 齐全）是读操作，仍由 skill 做。三者原子化。失败候选不进 changelog。P3 表述调整不升版本号：不打新 tag，changelog 在当前版本条目下追加带日期的调整小节。
- 生产恶化时**提醒人执行回滚**（切回上一版本 tag + 追加回滚条目），带新证据重入问题归因。
...
| git 分工 | 写/读分治：写（init/commit/tag/rollback）交人执行、skill 在守卫点提醒（commit 是检查点、3.4 出口，tag 是定稿、3.6 门禁后）；读（archive/diff/log）skill 自动。失败候选留在历史作重试证据；回滚点 = 上一版本 tag |
...
references/
  ...
  04-implementation.md   执行方案、最小修改、偏离记录、git 提交提醒
