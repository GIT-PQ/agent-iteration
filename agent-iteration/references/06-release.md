...
要么全部完成，要么全部不生效。git 没有跨 tag 与 commit 的原子操作：tag 已打而 changelog 写入失败时，由人删掉刚打的 tag 整体重来——changelog 缺条目即该版本未发布。

### 1. 打 tag

打 tag 由**人执行**——门禁八条全过后 skill 在此提醒。候选转正式版本，旧版自动转为回滚点。tag 指向**验证过的候选 commit**——与 05 验证报告记录的 hash 一致，验证对象 = 发布对象。
...
在工件仓库 `CHANGELOG.md` **顶部追加**一条（不新建文件），在 tag 打向后追加、由人提交。**发布时点一次生成**——失败候选不进 changelog，changelog 只记录定稿事实，不是意图。
...
回滚不在活动序列里：它发生在交付之后、沿"生产恶化"边离开发布时执行，直接操作版本库、不依赖流程状态。回滚的写操作由**人执行**，skill 在生产恶化时提醒：

1. 确认回滚点存在且可用（门禁第 8 条）——读操作，由 skill 做
2. 提醒人切换 tag 指向回滚点
3. 提醒人在 `CHANGELOG.md` 追加回滚条目（回滚至、原因、触发证据指针、后续任务编号）并提交——不删除被回滚版本的条目
